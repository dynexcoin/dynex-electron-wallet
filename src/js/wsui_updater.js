...
function setCircleSyncPercent(percent) {
	...
	Array.from(circleProgressPercent.classList).forEach((cls)=>{
		if (cls[0] == 'p') {
			circleProgressPercent.classList.remove(cls);
		}
	});

	// add current percent
	circleProgressPercent.classList.add('p' + percent);
	circleProgressText.innerHTML = percent + '%';
}
...
function updateSyncProgress(data){
    ...
    if(knownBlockCount === SYNC_STATUS_NET_CONNECTED){
		...
		let percent = parseInt((blockCount * 100) / knownBlockCount) || 0;
		// let percent = Math.min(100, parseInt((blockCount * 100) / knownBlockCount));
		// percent = Math.max(0, percent);
		setCircleSyncPercent(percent);
    }
